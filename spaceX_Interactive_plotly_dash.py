...
def get_scatter_plot(val1, val2):

    if val1 == 'ALL':
        filtered_df = spacex_df
        fig = px.scatter(filtered_df, x = "Payload Mass (kg)", y = "class",  color="Booster Version Category",
        title='Corrolation between payload ans success for all Sites')
        return fig
    else:
        filtered_df = spacex_df[spacex_df['Launch Site']==val1]
        filtered_df = filtered_df[filtered_df['Payload Mass (kg)'] >= val2[0]]
        filtered_df = filtered_df[filtered_df['Payload Mass (kg)'] <= val2[1]]
        fig = px.scatter(filtered_df, x = "Payload Mass (kg)", y = "class",  color="Booster Version Category",
        title='Corrolation between payload ans success for Site ' + val1)
        return fig       
